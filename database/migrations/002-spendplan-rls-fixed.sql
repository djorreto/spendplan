...
-- ========================================
-- Versión corregida para Supabase (funciones en schema public)

-- ========================================
-- FUNCIONES HELPER (en public, no auth)
-- ========================================
...
CREATE POLICY "Users can view profiles of household members"
    ...
    USING (
        id IN (
            ...
            WHERE hm.household_id = ANY(public.user_household_ids())
...
CREATE POLICY "Members can update household budgets"
    ...
    USING (public.is_household_member(household_id) AND NOT is_locked);
...
CREATE POLICY "Members can view budget lines"
    ...
    USING (
        EXISTS (
            ...
            AND public.is_household_member(mb.household_id)
        )
    );
...
CREATE POLICY "Members can view reconciliation links"
    ...
    USING (
        EXISTS (
            ...
            AND public.is_household_member(bt.household_id)
        )
    );
...
CREATE POLICY "Owners can view invitations"
    ...
    USING (public.is_household_owner(household_id) OR email = auth.email());
